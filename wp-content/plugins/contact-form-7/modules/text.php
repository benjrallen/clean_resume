...
function wpcf7_text_shortcode_handler( $tag ) {
	...
	foreach ( $options as $option ) {
		...
		} elseif ( preg_match( '%^inputtype:([-0-9a-zA-Z_]+)$%', $option, $matches ) ) {
			$inputtype_att = $matches[1];
	...
	if ( $inputtype_att )
		$atts .= ' type="' . $inputtype_att . '"';
	else
		$atts .= ' type="text"'; // default size
	...
	if ( $title_att )
		$atts .= sprintf( ' placeholder="%s"', trim( esc_attr( $title_att ) ) );

	$html = '<input name="' . $name . '" value="' . esc_attr( $value ) . '"' . $atts . ' />';
...
<tr>
<td><code>html5 input type? (<a href="http://www.456bereastreet.com/archive/201004/html5_input_types/" target="_blank" title="Input Types">Full list here</a>)</code> (<?php echo esc_html( __( 'optional', 'wpcf7' ) ); ?>)<br />
<input type="text" name="inputtype" class="inputtypevalue oneline option" /></td>
</tr>
...
<td>
<br /><input type="checkbox" name="watermark" class="option" />&nbsp;<?php echo esc_html( __( 'Use this text as HTML5 Placeholder?', 'wpcf7' ) ); ?>
</td>
</tr>
</table>
